...
def calibrate_motors(odrv):
    ...
    try:
        ...
        for axis_num in [0, 1]:
            ...
            # Calibrate encoder
            print(f"Calibrating encoder for {axis_name}...")
            ...
            print(f"Encoder calibration for {axis_name} successful.")
            
            # Additional encoder offset calibration
            print(f"Running encoder offset calibration for {axis_name}...")
            axis.requested_state = AXIS_STATE_ENCODER_OFFSET_CALIBRATION
            
            # Wait for offset calibration to complete
            print("Waiting for offset calibration to complete...")
            while axis.current_state != AXIS_STATE_IDLE:
                time.sleep(0.1)
                
            if axis.encoder.error != 0:
                print(f"Encoder offset calibration failed with error: {axis.encoder.error}")
                return False
                
            print(f"Encoder offset calibration for {axis_name} successful.")

        # Save configuration after successful calibration
    ...
    try:
        ...
        for axis_num in [0, 1]:
            ...
            if axis.current_state != AXIS_STATE_CLOSED_LOOP_CONTROL:
                print(f"Failed to enter closed-loop control mode for {axis_name}.")
                return False
...
def show_diagnostics(odrv):
    ...
    try:
        ...
        print(f"Brake Resistor: {'Enabled' if odrv.config.enable_brake_resistor else 'Disabled'}")
...
def main():
    ...
    # Default configuration for 1kW BLDC motors - adjust these values for your specific motors
    default_config = {
        'pole_pairs': 7,                # Motor pole pairs (adjust for your motors)
        'calib_voltage': 12.0,          # Calibration voltage
